...
static Node* new_node( Airport index, float weight )
{
   Node* n = (Node*) malloc( sizeof( Node ) );
   if( n != NULL )
   {
      n->data = index;
      n->data.travel_cost = weight;
...
void List_Push_back( List* list, Airport data, float weight )
{
...
Airport List_Cursor_get( List* list )
{
   assert( list->cursor );
